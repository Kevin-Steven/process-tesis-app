...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['tema_id']) && isset($_POST['revisor_id'])) {
    ...
    if ($result->num_rows > 0) {
        ...
        try {
            ...
            header("Location: tabla-revisor-tesis.php?status=success");
            exit();
        } catch (Exception $e) {
            ...
            header("Location: tabla-revisor-tesis.php?status=error");
            exit();
        }
    } else {
    ...
    header("Location: tabla-revisor-tesis.php?status=invalid_request");
    exit();
}
