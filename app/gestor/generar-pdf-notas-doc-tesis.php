...
$pdf->Cell(0, 10, 'Listado de Notas', 0, 1, 'C');
...
while ($row = $result->fetch_assoc()) {
    ...
    $pareja = ($row['pareja_nombres'] && $row['pareja_apellidos']) 
        ...
        : 'No aplica';
    $tema = $row['tema'];
    $nota = isset($row['nota']) && $row['nota'] !== null ? $row['nota'] : 'Sin nota';

    // Agregar la fila a la tabla
    $pdf->MultiCellRow([ $tema, $nota], $widths, $height);
}
