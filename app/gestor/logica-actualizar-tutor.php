...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['tema_id']) && isset($_POST['tutor_id'])) {
    ...
    if ($nuevo_tutor_id > 0) {
        ...
        if ($result->num_rows > 0) {
            ...
            // Redireccionar con éxito a detalle-tema.php con el ID del tema
            header("Location: detalle-tema.php?id=$tema_id&status=success");
            exit();
        } else {
            // Tema no encontrado
            header("Location: detalle-tema.php?status=not_found");
            exit();
        }
    } else {
        // Tutor no válido
        header("Location: detalle-tema.php?id=$tema_id&status=invalid_tutor");
        exit();
    }
} else {
    // Si no se enviaron los datos correctamente
    header("Location: detalle-tema.php?status=form_error");
    exit();
}
