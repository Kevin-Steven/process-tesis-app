...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['tema_id']) && isset($_POST['revisor_id'])) {
    ...
    if ($result->num_rows > 0) {
        ...
        try {
            ...
            $sql_update = "UPDATE tema SET id_revisor_plagio = ? WHERE usuario_id = ? AND estado_registro = 0";
            ...
            header("Location: tabla-revisor-plagio.php?status=success");
            exit();
        } catch (Exception $e) {
            ...
            header("Location: tabla-revisor-plagio.php?status=error");
            exit();
        }
    } else {
        // Si no se encontró el tema o está borrado lógicamente, redireccionar con un mensaje de error
        header("Location: tabla-revisor-plagio.php?status=not_found");
        exit();
    }
    ...
    header("Location: tabla-revisor-plagio.php?status=invalid_request");
    exit();
}
