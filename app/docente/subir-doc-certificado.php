...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $ruta_base = '../uploads/certificados/';
    ...
    if (!$conn) {
        header("Location: certificados.php?estado=error_conexion");
        exit();
    }
    ...
    if ($accion === 'subir' || $accion === 'editar') {
        if (isset($_FILES['archivo_tesis']) && $_FILES['archivo_tesis']['error'] === UPLOAD_ERR_OK) {
            ...
            if (!in_array(strtolower($extension), $allowedFileExtensions)) {
                header("Location: certificados.php?estado=error_tipo_archivo");
                exit();
            }

            // Validar tamaño del archivo
            if ($archivo['size'] > $maxFileSize) {
                header("Location: certificados.php?estado=error_tamano_archivo");
                exit();
            }

            // Generar nombre único para el archivo y moverlo a la carpeta
            $nombre_unico = uniqid('certificado_', true) . '.' . $extension;
            $ruta_destino = $ruta_base . $nombre_unico;

            if (!move_uploaded_file($archivo['tmp_name'], $ruta_destino)) {
                header("Location: certificados.php?estado=error_subida");
                exit();
            }

            // Actualizar la base de datos con la nueva ruta del archivo
            $sql = "UPDATE tema SET certificados = ? WHERE id = ?";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("si", $ruta_destino, $tesis_id);

            if ($stmt->execute()) {
                header("Location: certificados.php?estado=exito");
            } else {
                header("Location: certificados.php?estado=error_bd");
            }
            $stmt->close();
        } else {
            header("Location: certificados.php?estado=error_archivo");
        }
    } elseif ($accion === 'eliminar') {
        // Actualizar la base de datos para eliminar la referencia al archivo
        $sql = "UPDATE tema SET certificados = NULL WHERE id = ?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("i", $tesis_id);

        if ($stmt->execute()) {
            header("Location: certificados.php?estado=exito_eliminar");
        } else {
            header("Location: certificados.php?estado=error_bd");
        }
        $stmt->close();
