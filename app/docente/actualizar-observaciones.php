...
if ($_SERVER['REQUEST_METHOD'] !== 'POST' || !isset($_POST['enviar_observaciones'])) {
    header("Location: revisar-anteproyecto.php?status=form_error");
    exit();
}

$anteproyecto_id = $_POST['id_anteproyecto'];
...
if ($result->num_rows > 0) {
    ...
    header("Location: revisar-anteproyecto.php?status=not_found");
    exit();
}

$uploadDir = '../uploads/observaciones/';
...
if (isset($_FILES['archivo_observaciones']) && $_FILES['archivo_observaciones']['error'] === UPLOAD_ERR_OK) {
    ...
    if (!in_array($fileExtension, $allowedfileExtensions)) {
        ...
        if (move_uploaded_file($fileTmpPath, $destPath)) {
            ...
            header("Location: obs-realizadas-anteproyecto.php?status=success");
            exit();
        } else {
            $error = 'upload_error';
        }
    ...
    if ($error) {
        header("Location: obs-realizadas-anteproyecto.php?status=$error");
        exit();
    }
} else {
    header("Location: obs-realizadas-anteproyecto.php?status=no_file");
    exit();
}
